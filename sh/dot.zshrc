...
function prompt_setup() {
  ...
  PS1="
$t_cwd$t_shlvl\$(prompt-git-head-name)
$t_main"
  PS_DECORATOR="(*'-')/"
}
...
# ghq-peco lists repositories with peco and move to a selected repository.
ghq-peco() {
  local selected_dir=$(ghq list --full-path | sed "s,^$HOME,~," | peco --query "$LBUFFER")
